...
//        )->sendQueue();
    ...
    public function home(): void
    {
         ...
         * Atualizar usuário
         */
//        $user = (new User())->findByEmail("lucasfrancisco1318@gmail.com");
    ...
    public function login(?array $data): void
    {
        ...
        if (!empty($data['csrf'])) {
            ...
            if (request_limit("weblogin", 3, 60 * 5)) {
                $json['message'] = $this->message->error("Você já efetuou 3 tentativas, esse é o limit. Por favor, aguarde por 5 minutos para tentar novamente!")->render();
                echo json_encode($json);
                return;
            }
    ...
    public function forget(?array $data)
    {
        ...
        if (!empty($data['csrf'])) {
            ...
            if (request_repeat("webforget", $data["email"])) {
                $json['message'] = $this->message->error("Oooops! Você já tentou este e-mail antes")->render();
                echo json_encode($json);
                return;
            }
