...
function url(string $path = null): string
{
    ...
    if ($path) {
        return CONF_URL_PROD . "/" . ($path[0] == "/" ? mb_substr($path, 1) : $path);
    }
...
function theme(string $path = null, string $theme = CONF_VIEW_THEME): string
{
    if (strpos($_SERVER['HTTP_HOST'], "localhost")) {
        if ($path) {
            return CONF_URL_DEV . "/themes/{$theme}/" . ($path[0] == "/" ? mb_substr($path, 1) : $path);
        }
        return CONF_URL_DEV . "/themes/{$theme}";
    }

    if ($path) {
        return CONF_URL_PROD . "/themes/{$theme}/" . ($path[0] == "/" ? mb_substr($path, 1) : $path);
    }

    return CONF_URL_PROD . "/themes/{$theme}";
}
